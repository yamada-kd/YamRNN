...
class Network(tf.keras.layers.Layer):
	def __init__(self):
		...
		self.lstm=tf.keras.layers.RNN(MyGRUCell(5))
...
class MyGRUCell(tf.keras.layers.AbstractRNNCell,recurrent.DropoutRNNCellMixin):
	...
	def build(self,input_shape):
		...
		if self.use_bias:
			self.bias=self.add_weight(shape=(self.units*3,),name="bias",initializer=self.bias_initializer,regularizer=self.bias_regularizer,constraint=self.bias_constraint)
		else:
			self.bias=None
		self.built=True
	...
	def call(self,inputs,states,training=None):
		...
		rec_dp_mask=self.get_recurrent_dropout_mask_for_cell(vh,training,count=3)
		...
		v3=          self.activation(p31+K.dot(v2*vh3,self.recurrent_kernel[:,2*self.units:]))
		vh=(1-v1)*vh+v1*v3
		return vh,[vh]
