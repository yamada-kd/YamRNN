...
#	rnn=tf.keras.layers.RNN(YamRNNCell(4),return_sequences=True)
...
class Network(tf.keras.layers.Layer):
	def __init__(self):
		...
		self.lstm=tf.keras.layers.RNN(YamRNNCell(10))
...
class YamRNNCell(tf.keras.layers.AbstractRNNCell,recurrent.DropoutRNNCellMixin):
	def __init__(self,units,activation="tanh",recurrent_activation="sigmoid",use_bias=True,kernel_initializer="glorot_uniform",recurrent_initializer="orthogonal",bias_initializer="zeros",kernel_regularizer=None,recurrent_regularizer=None,bias_regularizer=None,kernel_constraint=None,recurrent_constraint=None,bias_constraint=None,dropout=0.,recurrent_dropout=0.,**kwargs):
		super(YamRNNCell,self).__init__(**kwargs)
		self.units=units
	...
	def call(self,inputs,states,training=None):
		...
		v1=self.recurrent_activation(p11+K.dot(vh,self.recurrent_kernel))
		v2=self.activation(p21)
		vh=(1-v2)-vh*v1-v2+v2*v2*(1-v1)
		return vh,[vh]
	...
	def get_config(self):
		...
		base_config=super(YamRNNCell,self).get_config()
