...
def main():
	...
	W2=theano.shared(value=np.asarray(np.random.uniform(low=-0.2,high=0.2,size=(RNNUNITSIZE,OUTPUTSIZE)),dtype=theano.config.floatX),name="W2")
	b2=theano.shared(value=np.asarray(np.random.uniform(low=-0.2,high=0.2,size=(OUTPUTSIZE)),dtype=theano.config.floatX),name="b2")
